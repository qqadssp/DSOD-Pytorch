...
class SSDBoxCoder:
    ...
    def encode(self, boxes, labels):
        ...
        if mask.any():
            extra = ious[mask.nonzero().squeeze()]
            if len(extra.size()) == 1 : extra = extra.view(1, -1)
            index[mask] = extra.max(1)[1]
    ...
    def decode(self, loc_preds, cls_preds, score_thresh=0.5, nms_thresh=0.5):
        ...
        for i in range(num_classes-1):
            ...
            if len(box.size()) == 1 : box = box.view(1, -1)
            score = score[mask]
        ...
        if len(boxes)==0:return torch.tensor([[0, 0, 0, 0]]), torch.tensor([0]), torch.tensor([0])
